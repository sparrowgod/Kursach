...
namespace kursach
{
    public partial class add_customer : Form
    {
        ...
        private void btn_add_Click(object sender, EventArgs e)
        {
            ...
            var addQuery = $"declare @js nvarchar(max) = (select {FIO} [fio]," +
                                                              $" {address} [address]," +
                                                              $" {phone} [phone] for json path, without_array_wrapper)," +
                           $" @rp nvarchar(max) exec [dbo].[ms_api] 'customer.create', @js, @rp out select @rp as [result]";
        ...
        private void add_customer_Load_1(object sender, EventArgs e)
        {
